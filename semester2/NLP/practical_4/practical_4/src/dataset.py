...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        tr_len = np.random.randint(4, int(self.block_size*7/8)+1)
        tr_len = min(tr_len, len(document))
        tr_document = document[:tr_len]

        mask_len = np.random.randint(int(tr_len*1/8), int(tr_len*3/8)+1)
        prefix_len = np.random.randint(1, tr_len-mask_len)

        prefix = tr_document[:prefix_len]
        masked_content = tr_document[prefix_len: prefix_len+mask_len]
        suffix = tr_document[prefix_len+mask_len:]

        masked_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content\
                        + self.PAD_CHAR * (self.block_size - tr_len - 2)

        input = masked_string[:-1]
        output = masked_string[1:]

        x = torch.LongTensor([self.stoi[c] for c in input])
        y = torch.LongTensor([self.stoi[c] for c in output])
