...
class NMT(nn.Module):
    ...
    def encode(
        ...
        dec_init_h = self.h_projection(torch.cat([last_hidden[0], last_hidden[1]], dim=-1))
        dec_init_c = self.h_projection(torch.cat([last_cell[0], last_cell[1]], dim=-1))
        dec_init_state = (dec_init_h, dec_init_c)
        # END YOUR CODE
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in torch.split(Y, split_size_or_sections=1):
            ...
            dec_state, out, e_t = self.step(Ybar_t, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        dec_hidden_i_unsqueezed = torch.unsqueeze(dec_hidden, dim=2)
        e_t = torch.bmm(enc_hiddens_proj, dec_hidden_i_unsqueezed)
        ...
        alpha_t_unsqeezed = torch.unsqueeze(alpha_t, dim=1)
        a_t = torch.bmm(alpha_t_unsqeezed, enc_hiddens)
