...
def naive_softmax_loss_and_gradient(
    ...
    probs_hat = probs
    probs_hat[outsideWordIdx] -= 1

    gradCenterVec = outsideVectors.T @ probs_hat
    gradOutsideVecs = np.outer(probs_hat, centerWordVec)
...
def neg_sampling_loss_and_gradient(
    ...
    for ind in indices[1:]:
        ...
        sigm = sigmoid(-np.dot(u_k, centerWordVec))

        loss -= np.log(sigm)
        gradCenterVec += - u_k * (sigm - 1)
        gradOutsideVecs[ind] += - centerWordVec * (sigm - 1)
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for word in outsideWords:
        ...
        loss_b, gradCenterVecs_b, gradOutsideVectors_b = word2vecLossAndGradient(centerWordVec, outsideWordIdx,
                                                                                 outsideVectors, dataset)
        loss += loss_b
        gradCenterVecs[word2Ind[currentCenterWord]] += gradCenterVecs_b
        gradOutsideVectors += gradOutsideVectors_b
